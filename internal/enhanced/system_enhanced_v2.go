...
func (es *EnhancedSystemV2) generateResponse(provider *selection.Provider, request string, confidence float64) *ProcessResponse {
	...
	taskType := es.selector.GetTaskType(request)
...
// GetTaskType is a helper method to expose task type detection
func (es *EnhancedSystemV2) GetTaskType(request string) selection.TaskType {
	return es.selector.GetDetector().DetectTaskType(request)
}
